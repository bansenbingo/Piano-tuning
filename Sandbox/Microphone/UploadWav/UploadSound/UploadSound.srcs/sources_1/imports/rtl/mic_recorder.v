...
//   3. SW[14] 上拨将全部已录音数据通过 UART 发送到计算机，下拨中止发送。
//
// 数据路径：
...
module mic_recorder #(
    ...
    reg sw_14_sync;
    reg sw_14_debounced;
    reg sw_14_debounced_d;
    reg [DEBOUNCE_W-1:0] sw_14_debounce_count;
    ...
    always @(posedge CLK100MHZ) begin
        if (rst) begin
            ...
            sw_14_debounced      <= 1'b0;
            sw_14_debounced_d    <= 1'b0;
            sw_14_debounce_count <= 0;
            sw_15_debounced      <= 1'b0;
            sw_15_debounced_d    <= 1'b0;
            sw_15_debounce_count <= 0;
        end else begin
            sw_0_debounced_d  <= sw_0_debounced;
            sw_14_debounced_d <= sw_14_debounced;
            ...
            if (sw_14_sync == sw_14_debounced) begin
                sw_14_debounce_count <= 0;
            end else if (sw_14_debounce_count == SWITCH_DEBOUNCE_CYCLES - 1) begin
                sw_14_debounced      <= sw_14_sync;
                sw_14_debounce_count <= 0;
            end else begin
                sw_14_debounce_count <= sw_14_debounce_count + 1'b1;
            end
    ...
    assign send_start   = sw_14_debounced && !sw_14_debounced_d;
